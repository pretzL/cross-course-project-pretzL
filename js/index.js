const API_URL = "https://api.rawg.io/api/games?key=35f9fd70b7b54c25bfa1662ebdeaff60";

const hotContainer = document.querySelector(".content-hot");
const errorContainer = document.querySelector(".error-container");

async function getGames() {
  try {
    const response = await fetch(API_URL);
    const results = await response.json();
    console.log(results);

    const games = results.results;

    hotContainer.innerHTML = "";

    for (let i = 0; i < games.length; i++) {
      if (i === 4) {
        break;
      }

      hotContainer.innerHTML += `<a href="/details.html?id=${games[i].id}" class="card">
      <img src="${games[i].background_image}" class="card-image"/>
        <h3>${games[i].name}</h3>
        <p>Rating: ${games[i].rating}</p>
        <p>Released: ${games[i].released}</p>
        </a>`;
    }
  } catch (error) {
    console.log(error);
    errorContainer.innerHTML = errorMessage("An error occurred when calling the API, error: " + error);
  }
}

getGames();

const URL = "https://api.rawg.io/api/games?dates=2019-10-10,2020-10-10&ordering=-added?key=35f9fd70b7b54c25bfa1662ebdeaff60";

const corsFix = "https://noroffcors.herokuapp.com/" + URL;

const comingSoonContainer = document.querySelector(".content-coming-soon");

async function getUpcomingGames() {
  try {
    const response = await fetch(corsFix);
    const results = await response.json();
    console.log(results);

    const games = results.results;

    hotContainer.innerHTML = "";

    for (let i = 0; i < games.length; i++) {
      if (i === 4) {
        break;
      }

      comingSoonContainer.innerHTML += `<a href="/details.html?id=${games[i].id}" class="card">
        <img src="${games[i].background_image}" class="card-image"/>
          <h3>${games[i].name}</h3>
          <p>Rating: ${games[i].rating}</p>
          <p>Released: ${games[i].released}</p>
          </a>`;
    }
  } catch (error) {
    console.log(error);
    errorContainer.innerHTML = errorMessage("An error occurred when calling the API, error: " + error);
  }
}

getUpcomingGames();
